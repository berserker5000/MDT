...
Function InitializeComputerName
	...
	ComputerDescription.Value = oEnvironment.Item("AssetTag") 
	
	If oProperties("OSDComputerName") = "" then
		OSDComputerName.Value = oUtility.ComputerName
	End if
	
	If oProperties("SerialNumber") = "" then
		SerialNumber.Value = oEnvironment.Item("SerialNumber") 
	End if
...
Function InitializeInventoryNumber
		If Len(ComputerDescription.value) = 13 then
		OSDComputerName.value = Left(OSDComputerName.Value, 3) & Mid(ComputerDescription.value ,4 ,6)	
		End If
End Function
